...
def words_to_morsecode(message):
    cipher =''
    for word in message:
        if word  != ' ':
            cipher += MORSE_CODE_DICT[word] + ' '
        else:
            cipher +=''
...
def morsecode_to_words(message):
    message += ' '
    decipher = ''
    citext = ''
    words = 0
...
def main():
    message1 = ("Hi-There")
    result = words_to_morsecode(message1.upper())
    ...
    result = morseCode_to_words(message2)
